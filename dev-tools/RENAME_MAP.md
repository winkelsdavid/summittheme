...
- [~] **T3 — in Arbeit.** **T3a ✅** (Labels: „Full Width"→„Section Width" 57×, „HTML / Description"→„Description" 38×, structure-aware). **T3b ✅** (44 kryptische Content-IDs → keyword-tragende Namen → `pool` statt AI-Pass; 37 Sections + 1 Snippet + 38 JSON, Saved-Werte migriert; Dry-Run 0 Kollisionen, word-boundary-safe). Bewusst gelassen: `text`/`name` (mehrdeutig) + Inzidental-Felder (Reviewer-Namen/Daten/Badges/Tabellen-Zellen) — keine Prime-Generierungs-Targets. **T3c ✅** (Block-Types) — erst revertiert (`1cf66c4`), dann **2026-06-15 korrekt neu angewandt + auf Frisch-Install verifiziert** (Blocks da). Die Revert-Begründung war eine **Fehldiagnose**: Shopify droppt platzierte Blocks **nicht** bei Type-Rename. Die leeren Sections im Content-Draft waren ein **Sync-Artefakt** (nur Schema synced, gespeicherte Block-Instanzen nicht mit-migriert → Mismatch). 18 Block-Types über 17 Sections → semantisch snake_case (`Item`→`slide`, `first_row`→`column_1`/`review_left`, `service`→`icon_item`…); 82 gespeicherte Instanzen structure-aware migriert (by-blockid, Doppelpunkt-Anchor); 0 Mismatches. **Deploy-Lektion (korrigiert):** wie bei allen Renames muss die App beim Update bestehender Themes die gespeicherten Daten mit-migrieren (Frisch-Install ist atomar).
